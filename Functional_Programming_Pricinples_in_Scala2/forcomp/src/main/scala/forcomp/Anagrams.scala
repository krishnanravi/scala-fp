...
object Anagrams extends AnagramsInterface {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.groupBy(c => c.toLower).transform((c, s) => s.length).toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    def helper(occurrences: Occurrences): List[Occurrences] = {
      occurrences match {
        case Nil => List(Nil)
        case head :: tail => {
          val tailCombinations = helper(tail)
          (for {
            count <- 1 to head._2
            tc <- tailCombinations
          } yield (head._1, count)  :: tc).toList ++ tailCombinations
        }
      }
    }
    helper(occurrences)
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    (y match {
      ...
      case _ => {
        val yMap = y.toMap
        for ((k, v) <- x) yield (k -> (v-yMap.getOrElse(k, 0)))
      }
